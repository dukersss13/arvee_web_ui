...
function authHeaders() {
    const headers = {};
    const token = localStorage.getItem("arveeAuthToken");
    const userEmail = localStorage.getItem("arveeUserEmail");
...
async function createSession({ clearUi = true } = {}) {
    ...
    try {
        const response = await arveeFetch("/api/session/new", { method: "POST" });
        const payload = await response.json().catch(() => ({}));
...
async function saveSession() {
    ...
    try {
        const response = await arveeFetch(`/api/session/${encodeURIComponent(sessionId)}/save`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
...
async function loadSessionInputs() {
    ...
    try {
        const response = await arveeFetch(`/api/session/${encodeURIComponent(sessionId)}`);
        const payload = await response.json();
        ...
        const stateResponse = await arveeFetch(
...
async function sendChatMessage() {
    ...
    try {
        const response = await arveeFetch("/api/chat/ask/stream", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
...
async function downloadValidatedPdf() {
    ...
    try {
        const response = await arveeFetch("/api/export/validated", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
